 ...
 * tsleep/ltsleep
 * how to use queues
 * pool_init/pool_get/pool_put/RUN_ONCE
 * callout_init
 * device_private
 * config_found_ia
 * pmf_device_register
...
struct atabus_initq_head atabus_initq_head =
    queue_init(atabus_initq_head);
mutex_t atabus_interlock = MUTEX_INITIALIZER;
...
ata_channel_attach(struct ata_channel *chp)
{
	...
	queue_init(&chp->ch_queue->queue_xfer);
