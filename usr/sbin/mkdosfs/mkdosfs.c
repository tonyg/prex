...
/* Crude Prex support by Tony Garnock-Jones, April 2011. Hacked up
   version 3.0.11 as shipped as part of dosfstools. */

/* Include the header files */

#include <sys/prex.h>
#define VERSION "3.0.11+" /* the "+" is for Prex modifications */
...
/* Prex doesn't have these in sys/mount.h */
#define BLOCK_SIZE_BITS 10
#define BLOCK_SIZE 1024

/* Prex doesn't yet have sufficient reflective facilities to easily support check_mount */
#define CAN_CHECK_MOUNT 0

/* assume Prex is little-endian */
#define CF_LE_W(v) (v)
#define CF_LE_L(v) (v)
#define CT_LE_W(v) (v)
#define CT_LE_L(v) (v)

/* Prex compiles in ansi mode, so inline doesn't exist */
#define inline

/* Prex doesn't define Linux's machine-word definitions. */
typedef uint8_t __u8;
typedef uint16_t __u16;
typedef uint32_t __u32;
typedef uint32_t u_int32_t; /* this one's just weird */

/* In earlier versions, an own llseek() was used, but glibc lseek() is
 * sufficient (or even better :) for 64 bit offsets in the meantime */
#define llseek lseek
typedef off_t loff_t; /* Prex doesn't have loff_t */
...
static device_t dev = 0;		/* FS block device file handle */
...
static off_t simulated_offset = 0; /* Prex: simulate seekable device */
...
static int valid_blkno (device_t dev, int blkno);
...
{
  long got;
  size_t size = try * BLOCK_SIZE;
  int error = device_read(dev, buffer, &size, current_block);
...
count_blocks (char *filename)
{
  ...
  error = device_open(filename, 0, &hdd);
  if (error) {
    fprintf(stderr, "Couldn't count_blocks: %s\n", strerror(error));
    exit(1);
  }

  /* Prex doesn't let you seek block devices */
  low = 0;
  for (high = 1; valid_blkno (hdd, high); high *= 2)
    low = high;
  while (low < high - 1) {
    const loff_t mid = (low + high) / 2;
    if (valid_blkno (hdd, mid))
      low = mid;
    else
      high = mid;
  }
...
establish_params (void)
{
  ...
  /* Prex doesn't have *any* support for disk geometry yet */
  bs.secs_track = CT_LE_W(32);	/*  Set up the geometry information */
  bs.heads = CT_LE_W(64);
...
#define seekto(pos,errstr)			\
  simulated_offset = pos

#define writebuf(buf,size,errstr)					\
  do {									\
    size_t __size = (size);						\
    errno = device_write(dev, buf, &__size, simulated_offset / sector_size); \
    if (errno)								\
...
{
  fatal_error("\
Usage: mkdosfs [-a][-A][-c][-v][-l bad-block-file][-b backup-boot-sector]\n\
...
main (int argc, char **argv)
{
  ...
  if (optind < argc)
	...
	printf("Device name doesn't start with '/dev/'.\n");
	usage();
      }
      device_name += 5;
  ...
  if (optind == argc - 2)	/*  Either check the user specified number */
    {
      blocks = strtoul (argv[optind + 1], &tmp, 0);
      if (blocks != cblocks)
	{
	  fprintf (stderr, "Warning: block count mismatch: ");
	  fprintf (stderr, "found %lu but assuming %lu.\n",
		   (unsigned long)cblocks,(unsigned long)blocks);
	}
    }
...
#endif
  error = device_open(device_name, 0, &dev);
  if (error) {
    fprintf (stderr, "%s: unable to open %s: %s\n", program_name, device_name, strerror(error));
    exit (1);			/* The error exit code is 1! */
  }
  ...
  establish_params ();		/* Establish the media parameters */
